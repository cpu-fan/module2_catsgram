...
public class PostFeedController {
    ...
    List<Post> getFriendsFeed(@RequestBody FeedParams feedParams) {
        if (!SORTS.contains(feedParams.getSort())) {
            throw new IncorrectParameterException("Передан некорректный параметр sort", feedParams.getSort());
        }
        if (feedParams.getFriendsEmails().isEmpty()) {
            throw new IncorrectParameterException("Передан некорректный параметр friends",
                    feedParams.getFriendsEmails().toString());
        }
        if (feedParams.getSize() == null || feedParams.getSize() <= 0) {
            throw new IncorrectParameterException("Передан некорректный параметр size", feedParams.getSize().toString());
        }
